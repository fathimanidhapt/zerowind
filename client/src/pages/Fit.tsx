...
export default function Fit() {
  const tech = services.find((t) => t.id === "fit") || services[0];
  const relatedProducts = products.filter((p) => p.technologyId === "fit");
  ...
  const scrollToContent = () => {
    const nextSection = document.getElementById("fit-details");
    if (nextSection) {
  ...
  return (
    <div className="bg-neutral-950 text-white min-h-screen relative overflow-hidden select-none">
      ...
      <AnimatePresence>
        {showSideWatermark && (
          <motion.div
            ...
            className="fixed left-[-100px] sm:left-[-85px] md:left-[-65px] top-[50vh] z-0 select-none pointer-events-none transform origin-center -rotate-90"
          >
            <img
              src="/images/download (9).svg"
              alt="FIT Watermark"
              className="h-12 sm:h-16 md:h-22 w-auto object-contain brightness-[0.5] opacity-15"
            />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div id="fit-details" className="max-w-7xl mx-auto px-6 md:px-12 py-24 relative z-10">
                            ...
                            <div className="space-y-6 flex-grow flex flex-col justify-between">
                              <div className="space-y-6">
                                ...
                                <p className="text-neutral-300 text-xs md:text-sm font-body leading-relaxed font-normal">
                                  Custom product exclusively for customer. {prod.description} Photo: Selkie jacket by Gobik (gobik.com)
                                </p>
                              </div>
                            </div>
                          </motion.div>
      ...
      <section className="relative w-full px-4 md:px-8 pt-10 md:pt-14 pb-24 flex justify-center bg-transparent">
        ...
        >
          {/* Background Image */}
          <img
            src="/images/imgi_12_shutterstock-560580814%401320x1020_r.jpeg.webp"
            alt="Discover Motion"
          ...
          <div className="relative z-10 flex flex-col items-center justify-center space-y-4 px-6 select-none">
            ...
            <img
              ...
              alt="MOTION"
              className="h-10 sm:h-14 md:h-18 w-auto object-contain brightness-200"
            />
            <span className="font-body text-white text-[10px] sm:text-xs font-light tracking-[0.4em] uppercase text-white/80">
              TECHNOLOGY
            </span>
