...
export default function Motion() {
    const tech = services.find((t) => t.id === "motion") || services[1];
    const relatedProducts = products.filter((p) => p.technologyId === "motion");
    ...
    const scrollToContent = () => {
        const nextSection = document.getElementById("motion-details");
        if (nextSection) {
      ...
      <AnimatePresence>
        {showSideWatermark && (
          <motion.div
            ...
            className="fixed left-[-220px] sm:left-[-200px] md:left-[-180px] top-[50vh] z-0 select-none pointer-events-none transform origin-center -rotate-90"
          >
            <img
              src="/images/download (2).svg"
              alt="MOTION Watermark"
              className="h-8 sm:h-11 md:h-15 w-auto object-contain brightness-[0.5] opacity-15"
            />
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <div id="motion-details" className="max-w-7xl mx-auto px-6 md:px-12 py-24 relative z-10">
                ...
                <div className="pl-0 md:pl-48 mb-28 w-full">
                    <div className="max-w-4xl space-y-6">
                        ...
                        >
                            High performance breathability, wind protection and waterproofness are guaranteed by a totally PFAS-free hydrophilic membrane, enclosed between two fabrics to create an ideal shell for even the toughest of the outdoor activities.
                        </motion.p>
                        <motion.p
                          initial={{ opacity: 0, y: 30 }}
                          whileInView={{ opacity: 1, y: 0 }}
            ...
            {/* 6. Power Promo Banner Section (Links to POWER) */}
            <section className="relative w-full px-4 md:px-8 pt-10 md:pt-14 pb-24 flex justify-center bg-transparent">
                ...
                >
                    {/* Background Image */}
                    <img
                        src="/images/imgi_13_istock-155467403%401320x1020_r.jpeg.webp"
                        alt="Discover Power"
